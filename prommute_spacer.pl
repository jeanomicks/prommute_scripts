...
($par{length} && $par{norg} && $par{ncyc} && $par{matrix} && $par{spacer} && $par{cutoff} && $par{infile} && $par{outfile}) or die $usage;
...
sub go {
	...
	my $ntfs = $#tfs;
	my $region_size = $par->{spacer};
	if ($ntfs > 1) {$region_size = ($ntfs - 1)*$par->{spacer};}
	...
	print OF "Spacer length: ".$par->{spacer}."\n";
	...
	for my $cyc (1 .. $par->{ncyc}) {
		...
		&mutate($organisms, $par->{norg}, $par->{length});
		# check if improved
		my $ok = &match_score($organisms,$par->{norg},\@tfs,$mx,\@motlens,\@maxscores,$par->{cutoff},$region_size);
		if ($ok == 1) {
			print OF "$cyc\n";
			exit(-1);
		}
...
sub match_score {
	my ($orgs, $norg, $tfs, $mx, $motlens, $maxscores, $cutoff, $rsize) = @_;
	my $ok = 0;
	for my $o (1 .. $norg) {
		$orgs->{$o}{score} = 0;
		my $ok2 = 0;
...
my $mt = substr($orgs->{$o}{promoter},$orgs->{$o}{$t}{pos},$motlen);
			if ($orgs->{$o}{$t}{pos} < $tf_pos_min) {$tf_pos_min = $orgs->{$o}{$t}{pos};}
			if ($orgs->{$o}{$t}{pos} > $tf_pos_max) {$tf_pos_max = $orgs->{$o}{$t}{pos};}
			if ($maxscore/($maxscores->[$motid]) >= $cutoff) {
				$ok2++;
			}
		}
		if (($ok2 == $#{$tfs}+1) && (($tf_pos_max - $tf_pos_min) <= $rsize)) {$ok = 1;}
		for my $t2 (0 .. $#{$tfs}) {
